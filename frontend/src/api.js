...
export const analyzeUrlTraffic = async (url) => {
    try {
        // Double check this path matches your Flask route exactly!
        const response = await axios.post(`${BASE_URL}/api/offline/analyze-url`, { url }, {
            headers: { 'Content-Type': 'application/json' }
        });
        return response.data;
    } catch (error) {
        // If the server sends a 404, error.response.data will contain the HTML page
        // We want to extract a cleaner message
        const message = error.response?.status === 404 
            ? "Endpoint /analyze-url not found on server. Check backend routes."
...
/**
 * Fetches the list of all available security reports
 */
export async function getReportsList() {
    return safeFetch(`${BASE_URL}/api/reports/list`);
}

/**
 * Fetches data for the Temporal Attack Trend chart
 */
export async function getAttackTrend() {
    return safeFetch(`${BASE_URL}/api/reports/trend`);
}

/**
 * Fetches data for the Categorical Weight pie chart
 */
export async function getReportDistribution() {
    return safeFetch(`${BASE_URL}/api/reports/distribution`);
}

/**
 * Dispatches an intelligence summary to a specific email
 * @param {string} email - The target analyst address
 */
export async function sendIntelligenceEmail(email) {
    return safeFetch(`${BASE_URL}/api/reports/email`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
