...
import { 
  Filter, Download, Trash2, RefreshCcw, Send, AlertTriangle, 
...
export default function ReportsPage() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // 2. Fetch Trend Data
      const trendData = await getAttackTrend();
      if (!trendData.error) setAttackTrend(trendData);

      // 3. Fetch Distribution Data
      const distData = await getReportDistribution();
      if (!distData.error) {
  ...
  const handleSendEmail = async () => {
    if (!email.trim() || !email.includes("@")) return toast.error("Valid target address required");
    setSending(true);
    const res = await sendIntelligenceEmail(email);
    if (!res.error) {
        toast.success(`Intelligence Summary dispatched to ${email}`);
        setEmail(""); // Clear on success
    } else {
        toast.error("Dispatch failure: " + res.error);
    }
    setSending(false);
  };
  ...
  return (
    <div className="relative p-8 space-y-8 bg-transparent min-h-screen text-slate-300 font-mono">
      <Toaster position="top-center" />

      {/* HEADER */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-800/50 pb-8 relative z-10">
        <div>
          <div className="flex items-center gap-2 text-cyan-400 text-[10px] uppercase tracking-[0.3em] mb-2 font-black">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="p-3 bg-white/5 border border-white/10 rounded-full hover:bg-cyan-500 hover:text-black transition-all"
          >
      ...
      <div className="grid lg:grid-cols-3 gap-6 relative z-10">
        <div className="lg:col-span-2 flex items-center gap-4 bg-slate-900/40 p-4 rounded-xl border border-white/5 backdrop-blur-sm">
            ...
            {["All", "Threat Intelligence", "Network Analysis", "System Health"].map((cat) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-md text-[10px] uppercase font-bold transition-all ${
                  filter === cat ? "bg-cyan-500 text-black" : "bg-white/5 text-slate-500 border border-white/5"
                }`}
              >
                {cat}
              </button>
        ...
        <div className="flex items-center gap-2 bg-slate-900/40 p-4 rounded-xl border border-white/5 backdrop-blur-sm">
          <input
            type="email"
            placeholder="Analyst Email Address"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="flex-1 bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-xs text-cyan-400 placeholder:text-slate-700 focus:outline-none focus:border-cyan-500"
          />
          <button
            onClick={handleSendEmail}
            disabled={sending}
            className="bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 p-2 rounded-lg hover:bg-cyan-500 hover:text-black transition-all"
          >
      ...
      <div className="grid lg:grid-cols-12 gap-6 relative z-10">
        <div className="lg:col-span-8 bg-slate-900/20 border border-white/5 rounded-2xl p-6 backdrop-blur-sm">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={attackTrend}>
              ...
              />
              <Line type="monotone" dataKey="value" stroke="#00e5ff" strokeWidth={3} dot={{ r: 4, fill: "#00e5ff" }} />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <div className="lg:col-span-4 bg-slate-900/20 border border-white/5 rounded-2xl p-6 flex flex-col items-center justify-center backdrop-blur-sm">
      ...
      <div className="space-y-4 relative z-10 pb-24">
        <h3 className="text-sm font-bold uppercase tracking-widest text-white flex items-center gap-2 px-2">
          <Database size={16} className="text-cyan-400" /> Data Repository
        </h3>
        
        {loading ? (
          <div className="h-48 flex items-center justify-center border border-dashed border-slate-800 rounded-2xl">
            <span className="animate-pulse text-slate-600 text-xs uppercase tracking-[0.5em]">Syncing_Intelligence_Data...</span>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredReports.map((r, i) => (
              <div key={i} className="group bg-slate-900/40 border border-white/5 p-5 rounded-2xl hover:border-cyan-500/40 transition-all backdrop-blur-sm">
                <div className="flex justify-between items-start mb-4">
                  <div className="p-3 bg-black/50 rounded-xl border border-white/5 text-cyan-400">
                    <FileSearch size={24} />
                  </div>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mt-1">
                    ...
                    <span className="text-[10px] text-cyan-400 bg-cyan-400/10 px-2 py-0.5 rounded uppercase font-bold">{r.size}</span>
                  </div>
                </div>
      ...
      {/* Floating Chat Assistant - High Z-index */}
